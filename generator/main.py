...
def trials():
    ...
    trial = random.choice(trial_names)
...
def interaction():
    members_arr = generate_random_array(candidates_fullnames_in_db, generate_random_integer(1,10))
    interaction_group = random.choice(character_groups)
    interaction_desc = random.choice(interactions)
...
def tournament():
    ...
    trials_group = random.choice(trial_group_descriptions)
...
def trial_in_process():
    return f"INSERT INTO trial_in_process (tournament_id, trial_id, candidate_id) VALUES (1, {generate_random_integer(1,30)}, {generate_random_integer(1,70)});"

def trials_hist():
    return f"INSERT INTO trials_history (candidate_id, trial_id, trial_status, tournament_id) VALUES ({generate_random_integer(1,70)}, {generate_random_integer(1,30)}, (select id from status where description = {sql_str_converter(random.choice(statuses_arr))}), 1);"
